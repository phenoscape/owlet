...
object ManchesterSyntaxClassExpressionParser {
	...
	class ManchesterParser(prefixes: Map[String, String]) extends RegexParsers {
		...
		def dataPropertyIRI: Parser[OWLDataProperty] = iri ^^ (factory.getOWLDataProperty(_));
		...
		val logger: Logger = Logger.getLogger(ManchesterSyntaxClassExpressionParser.getClass());
