...
object ManchesterSyntaxClassExpressionParser {
  ...
  private class ManchesterParser(prefixes: Map[String, String]) extends RegexParsers {
    ...
    def abbreviatedIRI: Parser[IRI] = PNAME_LN.r ^? { case PrefixAndLocalName(prefix, localName) => IRI.create(prefix + localName) }

    object PrefixAndLocalName {

      def unapply(text: String): Option[(String, String)] = {
        val segments = text.split(":", 2)
        val localName = if (segments.size > 1) segments(1) else ""
        prefixes.get(segments(0)).map((_, localName))
      }
